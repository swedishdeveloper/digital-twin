...
describe.only('maualdispatch', () => {
  ...
  it('should dispatch a booking to nearest car', function (done) {
    manualDispatch(cars, bookings).subscribe(({ car }) => {
  ...
  it('should dispatch two bookings even when they arrive async', function (done) {
    ...
    subscriptions.push(
      manualDispatch(cars, bookingStream).subscribe((booking) => {
        if (booking.id === '1') {
  ...
  it('should have cars available even the second time', function (done) {
    ...
    subscriptions.push(
      manualDispatch(cars, bookingStream).subscribe(({ id, car }) => {
        if (id === '1') {
          expect(car?.position).toEqual(ljusdal)
          bookingStream.next(
            new Booking({
